...
def update_sarsa(step_size, Q, state, action, reward, done, next_state, next_action, gamma):
    ...
    # TODO: 여기를 작성하세요.
    if done:
        return Q[state][action] + step_size * (reward - Q[state][action])
    else:
        return Q[state][action] + step_size * (reward + gamma * Q[next_state][next_action] - Q[state][action])
    ###################
...
def update_q_learning(step_size, Q, state, action, reward, done, next_state, gamma):
    ...
    if done:
        ...
        return Q[state][action] + step_size * (reward + gamma * np.max(Q[next_state, :] - Q[state][action]))
    ###################
