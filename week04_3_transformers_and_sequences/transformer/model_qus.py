...
def dot_scaled_attention(
    ...
    # All values in last dimension (d_k dimension) are zeros for <PAD> location
    assert (padding_mask == (query == 0.).all(-1)).all()
    assert (padding_mask == (key == 0.).all(-1)).all()
    assert (padding_mask == (value == 0.).all(-1)).all()

    # YOUR CODE HERE
    query = query.permute(1, 0, 2)  # from (sequence_length, batch_size, d_k) to (batch_size, sequence_length, d_k)
    key = key.permute(1, 2, 0)  # from (sequence_length, batch_size, d_k) to (batch_size, d_k, sequence_length)
    value = value.permute(1, 0, 2)
    qkt_score = torch.bmm(query, key)  # torch.bmm is for batch matrix multiplication
    normalized_qkt = qkt_score / math.sqrt(d_k)
    softmax_val = nn.Softmax(dim=-1)(normalized_qkt)  # dim=-1 means do softmax automatically
    p_attn = torch.bmm(softmax_val, value)
    attention = None
    # END YOUR CODE
